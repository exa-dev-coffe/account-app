# 🗄️ Database Migration Documentation (Liquibase) - `be-coffe-java`
...
└── changes/
    └── 001_create_all_tables.sql    # Liquibase SQL changeset migration script
```
...
1. Ensure PostgreSQL database connection variables are configured in `.env` or `application.properties`:
   ```properties
   SPRING_DATASOURCE_URL=jdbc:postgresql://localhost:5432/postgres-kios
...
1. Create a new `.sql` changeset file in `src/main/resources/db/changelog/changes/`:
   Example: `002_add_user_phone_column.sql`

2. Format the SQL file with Liquibase changeset metadata header:
   ```sql
   --liquibase formatted sql
   --changeset author_name:002_add_user_phone_column
...
3. Include the file in `src/main/resources/db/changelog/db.changelog-master.yaml`:
   ...
   databaseChangeLog:
     - include:
         file: db/changelog/changes/001_create_all_tables.sql
     - include:
         file: db/changelog/changes/002_add_user_phone_column.sql
   ```
