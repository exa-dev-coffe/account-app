...
-- precondition-sql-check expectedResult:0 SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'tm_roles';
...
(
    user_id    INTEGER GENERATED BY DEFAULT AS IDENTITY (START WITH 2) NOT NULL,
    created_at TIMESTAMP WITHOUT TIME ZONE DEFAULT NOW() NOT NULL,
    created_by INTEGER,
    updated_at TIMESTAMP WITHOUT TIME ZONE DEFAULT NOW() NOT NULL,
    updated_by INTEGER,
...
-- rollback DROP TABLE tm_refresh_tokens CASCADE;
-- rollback DROP TABLE tm_reset_token_passwords CASCADE;
-- rollback DROP TABLE tm_accounts CASCADE;
-- rollback DROP TABLE tm_roles CASCADE;
